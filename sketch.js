var play,about,playimg,aboutimg,back,backImg,howtoImg,nextImg,aboutbg,g1Img,g2Img,g3Img,g4Img,g5Img,liftImg,plrWR,plrWL,plrShotR,plrShotL,plrst,plrstl,bul1Img,bul2Img,heliImg
var bg1,bg2,bg3,bg4;
...
function preload(){
    ...
    howtoImg=loadImage("load/help.jpg")
    ...
    heliImg=loadImage("ground/helicop.png")
...
wall1.visible=false
var wall2=createSprite(windowWidth-5,windowHeight/2,5,700)
wall2.visible=false
...
if(gameState==="play2"){
    ...
    var lift1 = createSprite(windowWidth/5.2,windowHeight/1.5);
    lift1.addImage(liftImg);
    lift1.scale = 0.5;
    lift1.debug = true
    var lift2 = createSprite(windowWidth/1.4,windowHeight-50);
    ...
    wall9.visible = false
    var wall10 = createSprite(windowWidth/2,windowHeight-10,windowWidth,5)
    wall10.visible = false
    var wall11 = createSprite(windowWidth/2,windowHeight/13,windowWidth,5)
    wall11.visible = false
    var wall12 = createSprite(windowWidth/12,windowHeight/1.25,150,175)
    ...
    var wall15 = createSprite(windowWidth/1.1,windowHeight/1.7,220,500)
...
if(gameState==="play4"){
    ...
    var heli = createSprite(windowWidth/5,windowHeight/2.5) 
    heli.depth = 0.5
    heli.addImage(heliImg)
    heli.scale = 1
    heli.debug=true
    heli.setCollider("rectangle",-130,50,200,100)
    ...
    var enyd = createSprite(windowWidth/2,windowHeight/1.9)
    enyd.addImage(robo)
    enyd.scale = 0.4    
    var enye = createSprite(windowWidth/1.6,windowHeight-50)
    enye.addImage(robol)
    enye.scale = 0.4
    
    var enyhitd = createSprite(enyd.position.x +250,enyd.position.y-18,500,10)
    enyhitd.visible = false
    var enyhite = createSprite(enye.position.x -250,enye.position.y-18,500,10)
    enyhite.visible = false
    


    wlssss = [wall28,wall29,wall30,wall31,wall32,lift5,enyd,enye]
     
    for(var i = 0;i<8;i++){
         wall4Group.add(wlssss[i]);
       }

       var enydBullet = createSprite(enyd.position.x,enyhitd.position.y,10,1)
      enydBullet.visible=false
      enydBullet.addImage(bul1Img)
      enydBullet.scale = 0.1
      enydBullet.setCollider("rectangle",0,0,10,1)
      enydBullet.debug = true
      if(enyhitd.isTouching(plr)){
          enydBullet.visible=true;
          enydBullet.velocityX = 50
      ...
      enyBulletGroup.lifetime = 60;
      enyBulletGroup.add(enydBullet)

      
      var enyeBullet = createSprite(enye.position.x,enyhite.position.y,10,1)
      enyeBullet.visible=false
      enyeBullet.addImage(bul2Img)
      enyeBullet.scale = 0.1
      enyeBullet.setCollider("rectangle",0,0,10,1)
      enyeBullet.debug = true
      if(enyhite.isTouching(plr)){
          enyeBullet.visible=true;
          enyeBullet.velocityX = -50
          ss.play()
      }    
        if(plr.isTouching(enyBulletGroup)){
            enyBulletGroup.destroyEach()
      ...
      enyeBullet.lifetime= 60;
      enyBulletGroup.add(enyeBullet)
